...
class WebsitestatController extends Controller {
	...
	public function init() {
		...
		$this->captchaEnabled=Utils::isRecaptchaEnabled();
		$this->collectInfo();
	}
	...
	public function actionGenerateHTML($domain) {
		...
		//$hidePageSpeedInsightBlock = (is_array($this->pageSpeed) AND empty($this->pageSpeed));
        //$showPageSpeedInsightDownloadForm = (is_array($this->pageSpeed) AND !empty($this->pageSpeed));
		...
		$this->render('index', array(
            ...
            //"showPageSpeedInsightDownloadForm"=>$showPageSpeedInsightDownloadForm,
            //"hidePageSpeedInsightBlock"=>$hidePageSpeedInsightBlock,
            "misc"=>$this->misc,
            //"downloadPageSpeedPdfForm"=>$downloadPageSpeedPdfForm,
            "psiConfig"=>Utils::loadConfig("psi")
		));
	}
    ...
    protected function createPdfFromHtml($html, $pdfFile, $filename) {
        ...
        $pdf -> SetAuthor("http://website-review.codeine.xyz");
    ...
    /*public function actionGeneratePageSpeed($domain) {
        if($this->captchaEnabled OR empty($this->pageSpeed)) {
            throw new CHttpException(404, Yii::t("app", "The page you are looking for doesn't exists"));
        }
